...
- **Draft updates replace the message**: a `PUT` that changes content, attachments or `removeAttachments` rebuilds the draft, appends the new version and **permanently deletes** the old UID (it is not moved to Trash), so the UID changes — flag-only updates stay in place. `removeAttachments` takes existing attachment ids; unknown ids are a 400. `IMAPAccount.createMail` returns the new UID (UIDPLUS `APPENDUID`, else a lookup).
