...
- **Composing and sending**: `POST .../mails` takes a JSON body, or `multipart/form-data` with the mail as a JSON `mail` field plus repeated `attachments` files (attachments limited by `DLA_MAX_ATTACHMENT_SIZE_MB`, the whole request by that + 16 MB). Drafts keep their Bcc header in IMAP; `POST .../send` relays the stored raw source via `SMTPAccount.sendRaw`, which puts Bcc recipients in the envelope and strips the header. `PUT` content updates rebuild the draft and re-attach its existing attachments; flag-only updates stay in place. `IMAPAccount.createMail` returns the new UID (UIDPLUS `APPENDUID`, else a lookup).
